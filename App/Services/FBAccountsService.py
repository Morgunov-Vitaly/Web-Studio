...
from App.Services.FacebookGraphAPIService import FacebookGraphAPIService


class FBAccountsService:
    def __init__(self, log, settings, driver_path, email, password, number_of_inactive_ad_account):
        ...
        self.sleep_time = int(self.settings_service.settings['default']['SLEEP_TIME'])
        self.facebook_url = self.settings_service.settings['default']['FACEBOOK_URL']
        self.inactive_accounts_limit = self.settings_service.settings['default']['INACTIVE_ACCOUNTS_LIMIT_NOTIFICATION']

        # Browser settings_service
        self.options = Options()
        # Опция отвечает за фоновый режим работы браузера
        self.options.headless = bool(self.settings_service.settings['default']['HIDE_BROWSER'])
        print('options.headless')
        print(self.options.headless)
        self.options.binary_location = Path.normalise_path(
            self.settings_service.settings['default']['BROWSER_LOCATION']
        )
    ...
    def get_ad_manager_access_token(self):
        ...
        email_field = self.browser.find_element_by_id(
            self.settings_service.settings['default']['LOGIN_FORM_EMAIL_FIELD']
        )
        ...
        password_field = self.browser.find_element_by_id(
            self.settings_service.settings['default']['LOGIN_FORM_PASSWORD_FIELD']
        )
        ...
        if not submit:
            ...
            submit = self.browser.find_element_by_id(
                self.settings_service.settings['default']['LOGIN_FORM_SUBMIT_BUTTON']
            )
            if not submit:
    ...
    def process_request_from_bm_to_ads_manager(self, ad_account, ad_account_id):
        ...
        while adding_ads_to_business.get("access_status") != "PENDING":
            ...
            detailed_text = f'Ошибка: {error_message} - {error_user_title} : {error_user_msg}'
            self.log.error(detailed_text)
            message = 'Возникла ошибка при отправке запроса на добавление аккаунта в BM!'
            informative_text = 'Нажмите Ok для продолжения ' \
                               'или Abort для прекращения работы скрипта'
            self.log.debug(informative_text)
            detailed_text += 'Проверьте статусы запросов в BM аккаунте. ' \
                             'При необходимости отмените непринятые приглашения. ' \
                             'После устранения причины ошибки - нажмите Ok для продолжения ' \
                             'или Abort для прекращения работы скрипта'
            ...
            if x == self.settings_service.abort_button_code:
                self.log.error('Проблема с аккаунтом BM так и не была устранена.')
                self.log.error('Пользователь завершил работу скрипта.')
                self.log.debug(f'Закрываем браузер..')
                self.browser.quit()
        ...
        # self.browser.save_screenshot(Path.get_screenshots_path('_accept_request.png'))

        # Look up the business manager request
        element = self.browser.find_element_by_css_selector(
            self.settings_service.settings['default']['CONFIRM_REQUEST_BUTTON']
        )
        repeats = 0
        ...
        if not element:
            raise RequestFromBmToAdsManagerFault(
                f"'Не удалось найти кнопку (элемент "
                f"{self.settings_service.settings['default']['CONFIRM_REQUEST_BUTTON']} "
        ...
        request_accepting_link = self.browser.find_element_by_css_selector(
            self.settings_service.settings['default']['CONFIRM_REQUEST_LINK']).get_attribute('href')
        repeats = 0
        ...
        if not request_accepting_link:
            # Делаем скриншот экрана браузера
            # self.browser.save_screenshot(Path.get_screenshots_path('_modal_accept_link.png'))
            raise RequestFromBmToAdsManagerFault(
                f"Не удалось найти ссылку для подтверждения запроса! "
                f"Элемент {self.settings_service.settings['default']['CONFIRM_REQUEST_LINK']}"
            )
    ...
    def request_to_add_user_account_to_bm(self):
        ...
        # Создаем экземпляр класса FacebookGraphAPIService
        ad_account = FacebookGraphAPIService(self.access_token)
        ...
        for ads_account in ads_accounts['data']:
            ...
            if not ad_account.is_ad_account_active(ads_account['account_id']):
                ...
                if self.number_of_inactive_ad_account >= self.inactive_accounts_limit:
                    detailed_text = f'Кол-во обнаруженных неактивных рекламных аккаунтов клиентов ' \
                                    f'достигло лимита ({self.inactive_accounts_limit})!: ' \
                                    f'{self.number_of_inactive_ad_account}.' \
                                    f'После нажатия на Ok счетчик неактивных аккаунтов будет обнулен!'

                    self.log.warning(detailed_text)
                    message = f'Обнаружено {self.number_of_inactive_ad_account} неактивных аккаунтов!'
                    information_text = 'Смените IP и нажмите Ok чтобы продолжить!'

                    UiDialog.pause_continue_notification_dialog(message, information_text, detailed_text)
